...
import { signUp as apiSignUp } from "../api/controllers/auth";
...
export default function SignUp() {
  ...
  const { isPending: loading, mutate: signUp } = useMutation({
    mutationFn: apiSignUp,
    onSuccess() {
      toast.success(
        "Account created successfully. Kindly login with your details"
      );
      navigate("/login");
    },
    onError(error) {
    ...
    <main className="auth">
      <div className="auth__header">
        ...
        <p className="auth__sub-heading">
          Hey 👋, Fill in the details below to create your account.
        </p>
      </div>
      <div className="auth__oauth">
        <button
          onClick={() =>
            toast("Sign Up with google is not available at the moment")
          }
        >
          <svg
            width="24"
      ...
      <div className="auth__or">or Sign Up with Email</div>
      <form className="auth__form" onSubmit={handleSubmit(onSubmit)}>
        ...
        <FormRow error={errors.phoneNumber?.message}>
          <label htmlFor="phoneNumber" className="flabel">
            Phone Number
          </label>
          <input
            type="tel"
            placeholder="e.g +2347026712611"
            className="input"
            id="phoneNumber"
            disabled={loading}
